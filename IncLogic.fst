...
open FStar.FunctionalExtensionality  {[op_Hat_Subtraction_Greater ]}
...
let test () =
  ...
  assert (FE.feq #nat f1 f2); // assert (forall (x:nat). f1 x == f2 x);
...
let rec eval_expr (s: state) (e: expr) : GTot int =
  ...
  | Gt e1 e2 -> if eval_expr s e1 > eval_expr s e2 then 0 else 1
...
type runsto : p: stmt -> s0: state -> m: term_mode -> s1: state -> Type0 =
  ...
  | R_Ext :
    ...
    -> runsto p s0' m s1'
  | R_Assign : s: state -> #x: var -> #e: expr
    -> runsto (Assign x e) s Ok (override s x (eval_expr s e))
  | R_Nondet : s: state -> #x: var -> v: value -> runsto (Nondet x) s Ok (override s x v)
  | R_Skip : s: state -> runsto Skip s Ok s
  | R_Error : s: state -> runsto Error s Er s
  | R_Assume : s: state -> #e: expr -> squash (eval_expr s e == 0) -> runsto (Assume e) s Ok s
  | R_SeqEr : #p: stmt -> #q: stmt -> #s: state -> #t: state -> runsto p s Er t
    -> runsto (Seq p q) s Er t
  | R_Seq :
      #p: stmt ->
      #q: stmt ->
      #m: term_mode ->
  ...
  | R_ChoiceL : #p: stmt -> #q: stmt -> #s: state -> #m: term_mode -> #t: state -> runsto p s m t
    -> runsto (Choice p q) s m t
  | R_ChoiceR : #p: stmt -> #q: stmt -> #s: state -> #m: term_mode -> #t: state -> runsto q s m t
    -> runsto (Choice p q) s m t
  | R_Kleene0 : #p: stmt -> #s: state -> runsto (Kleene p) s Ok s
  | R_KleeneS :
      #p: stmt ->
      #s: state ->
      #m: term_mode ->
...
type il_triple : pre: cond -> p: stmt -> post_ok: cond -> post_er: cond -> Type =
  | I_Assign : #pre: cond -> #x: var -> #e: expr
    -> il_triple pre
        (Assign x e)
        (fun s ->
            exists x_init. pre (override s x x_init) /\ (s x = eval_expr (override s x x_init) e))
        (fun s -> false)
  | I_Nondet : #x: var -> #pre: cond
    -> il_triple pre (Nondet x) (fun s -> exists v. pre (override s x v)) (fun s -> false)
  | I_Skip : pre: cond -> il_triple pre Skip pre (fun s -> false) // | I_Local : 
  | I_Error : pre: cond -> il_triple pre Error (fun s -> false) pre
  ...
  | I_ChoiceR :
    ...
    -> il_triple pre (Choice p q) post_ok post_er
  | I_Kleene0 : #p: stmt -> #pre: cond -> il_triple pre (Kleene p) pre (fun s -> false)
  | I_KleeneS :
      #p: stmt ->
      #pre: cond ->
      #post_ok: cond ->
  ...
  | I_KleeneVariant :
      #variant: (nat -> cond) ->
      #p: stmt ->
      (n: nat -> il_triple (variant n) p (variant (n + 1)) (fun s -> false))
    -> il_triple (variant 0) (Kleene p) (fun s -> exists n. variant n s) (fun s -> false)
  | I_Empty : #pre: cond -> #p: stmt -> il_triple pre p (fun s -> false) (fun s -> false)
  | I_Consequence :
      #pre: cond ->
      #p: stmt ->
      #post_ok: cond ->
...
let rec soundness_ok
  ...
  | I_Assign #pre #x #e ->
    ...
    assert (exists x_init. pre (override s1 x x_init) /\ (s1 x = eval_expr (override s1 x x_init) e)
      );
    let x_init =
      FStar.IndefiniteDescription.indefinite_description_ghost _
        (fun x_init -> pre (override s1 x x_init) /\ (s1 x = eval_expr (override s1 x x_init) e))
    in
    assert (pre (override s1 x x_init) /\ (s1 x = eval_expr (override s1 x x_init) e));
  ...
  | I_Nondet #x #pre ->
    assert (p == Nondet x);
    assert (exists v. pre (override s1 x v)); // Existe v tal que pre s1[v/x]
    let v =
      // Tomar ese v.
      FStar.IndefiniteDescription.indefinite_description_ghost _ (fun v -> pre (override s1 x v))
    in
    assert (pre (override s1 x v));
    let s0 = override s1 x v in
    assert (pre s0);
    let pf0:// ^ Conseguimos un estado inicial.
    // Dado que podemos elegir el valor que toma x, elijamos (s1 x).
    // Entonces es claro que terminamos en s1, como necesitamos.
    runsto (Nondet x) s0 Ok (override s0 x (s1 x)) =
      R_Nondet s0 #x (s1 x)
    in
    let pf1:// Solamente hay que aplicar extensionalidad...
    runsto (Nondet x) s0 Ok s1 =
      R_Ext pf0 _ _
    in
    (| s0, pf1 |)
  | I_Skip _ ->
  ...
  | I_Seq #p #q #pre #mid_ok #mid_er #post_ok #post_er pf_p pf_q ->
    let (| s_mid , r_q |) = soundness_ok q mid_ok post_ok post_er pf_q s1 in
    let (| s0 , r_p |) = soundness_ok p pre mid_ok mid_er pf_p s_mid in
    let r = R_Seq r_p r_q in
    (| s0, r |)
  | I_ChoiceL #p #q #pre #post_ok #post_er pf_p ->
    let (| s0 , r_p |) = soundness_ok p pre post_ok post_er pf_p s1 in
    let r = R_ChoiceL #p #q r_p in
    (| s0, r |)
  | I_ChoiceR #p #q #pre #post_ok #post_er pf_q ->
    let (| s0 , r_q |) = soundness_ok q pre post_ok post_er pf_q s1 in
  ...
  | I_KleeneS #p #pre #post_ok #post_er pf_seq ->
    let (| s0 , r_seq |) = soundness_ok (Seq (Kleene p) p) pre post_ok post_er pf_seq s1 in
    let r = R_KleeneS #p r_seq in
    (| s0, r |)
  | I_KleeneVariant #variant #p pf_var -> admit ()
  | I_Empty -> unreachable ()
  | I_Consequence #pre #p #post_ok #post_er pre' post_ok' post_er' pf_p _ _ _ ->
    let (| s0 , r |) = soundness_ok p pre post_ok post_er pf_p s1 in
    (| s0, r |)
  | I_Disjunction #pre1 #pre2 #p #post_ok1 #post_ok2 #post_er1 #post_er2 pf_p1 pf_p2 ->
    if p2b (post_ok1 s1)
    then
  ...
  | I_Seq #p #q #pre #mid_ok #mid_er #post_ok #post_er pf_p pf_q ->
    ...
    then
      ...
      (assert (post_er s1);
        ...
        (| s0, R_Seq #p #q r1 r2 |))
  | I_ChoiceL #p #q #pre #post_ok #post_er pf_p ->
    let (| s0 , r_p |) = soundness_er p pre post_ok post_er pf_p s1 in
    let r = R_ChoiceL #p #q r_p in
    (| s0, r |)
  | I_ChoiceR #p #q #pre #post_ok #post_er pf_q ->
    let (| s0 , r_q |) = soundness_er q pre post_ok post_er pf_q s1 in
    let r = R_ChoiceR #p #q r_q in
    (| s0, r |)
  | I_Kleene0 -> unreachable ()
  | I_KleeneS #p #pre #post_ok #post_er pf_seq ->
    let (| s0 , r_seq |) = soundness_er (Seq (Kleene p) p) pre post_ok post_er pf_seq s1 in
    let r = R_KleeneS #p r_seq in
    (| s0, r |)
