...
let proof_client (ptr : var) : isl_triple (is_ok (pre_client ptr)) (client ptr) (post_err ptr) =
  ...
  let p_load_y_raw = ISL_Load #(post_load_x ptr) "y" (Var ptr) in
  let p_load_y = ISL_Consequence (post_load_x ptr) (post_load_y ptr) p_load_y_raw () () in
  ...
  let p_push_back = ISL_Consequence (post_load_x ptr) (post_store_v ptr) p_push_back_raw () () in

  let p_store_err_raw = ISL_StoreEr #(post_store_v ptr) (Var "x") (Const 88) in
  let p_store_err = ISL_Consequence (post_store_v ptr) (post_err ptr) p_store_err_raw () () in

  let p_client_seq2 = ISL_Seq p_push_back p_store_err in
  
  let p_client_raw : isl_triple (is_ok (pre_client ptr)) (client ptr) 
